...
export function WhatsAppActivation({ userEmail }: WhatsAppActivationProps) {
    ...
    return (
        <div className="bg-white/60 backdrop-blur-xl rounded-[2rem] p-6 border border-white/60 overflow-hidden relative group transition-all duration-500">
            {isActive ? (
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h3 className="text-sm font-bold text-gray-900">WhatsApp Activo</h3>
                            <p className="text-xs text-gray-500">
                                Hasta {new Date(expiresAt!).toLocaleString('es-AR', { hour: '2-digit', minute: '2-digit', day: '2-digit', month: 'short' })}
                            </p>
                        </div>
                    </div>
                    <div className="bg-green-50 text-green-700 text-[9px] font-bold uppercase tracking-wider px-3 py-1.5 rounded-full">
                        Activo
                    </div>
                </div>
            ) : (
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-gray-500">
                                {status === 'expired' ? "Ventana cerrada" : "No activado"}
                            </p>
                        </div>
                    </div>
                    <a
                        ...
                        className="bg-[#25D366] text-white px-4 py-2 rounded-2xl font-bold text-xs flex items-center gap-2 hover:bg-[#25D366]/90 transition-all"
                    >
